...
namespace GameProject
{
    ...
    class NumberBoard
    {
        ...
        // to store if correct number was clicked
        bool correctClicked;
            ...
            int correctNumber, SoundBank soundBank)
        {
            ...
            tileSideLength = (sideLength - 4 * BORDER_SIZE) / NUM_ROWS;


            // initialize array of number tiles
            for (int i = 0; i < NUM_ROWS; i++)
                for (int j = 0; j < NUM_COLUMNS; j++)
                    tiles[i, j] = new NumberTile(contentManager, CalculateTileCenter(i, j), tileSideLength, i * NUM_COLUMNS + j + 1, correctNumber, soundBank);
        ...
        public bool Update(GameTime gameTime, MouseState mouse)
        {
            ...
            for (int i = 0; i < NUM_ROWS; i++)
            {
                for (int j = 0; j < NUM_COLUMNS; j++)
                {
                    correctClicked = tiles[i, j].Update(gameTime, mouse);
                    if (correctClicked)
                        return true;
                }
            }
